...
```bash
make build; docker save go-api:latest -o go-api.tar; sudo ctr -n k8s.io images import go-api.tar; rm go-api.tar; make deploy; kubectl rollout restart deployment/go-api-deployment; sleep 24; kubectl logs -f  $(kubectl get pod -l app=go-api -o custom-columns=NAME:.metadata.name --no-headers)
```
